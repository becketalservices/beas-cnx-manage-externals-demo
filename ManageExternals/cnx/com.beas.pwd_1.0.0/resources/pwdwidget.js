define([
        ...
        'dijit/Dialog',
        'dojo/text!./templates/pwdwidget.html',
	...
	return declare("com.beas.pwd.pwdwidget", [_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, _Dialog], {
		version: '201507201800',
		...
		changePassword: function() {
			...
			if (formObject.oldpwd.length > 0 && formObject.newpwd1.length > 0 && formObject.newpwd2.length > 0 ) {
				if (formObject.newpwd1 == formObject.newpwd2) {
					this.postPassword(formObject.oldpwd, formObject.newpwd1, formObject.newpwd2);
				} else {
					this.writeMessage("error", this.nls.messages.nomatch);
				}
			} else {
		...
		cancelDialog: function() {
			this.writeMessage("","");
			dojo.byId(this.id + "_oldpwd").value="";
			dojo.byId(this.id + "_newpwd1").value="";
			dojo.byId(this.id + "_newpwd2").value="";
			this.onCancel();
		},
		...
		postPassword: function(oldpwd, newpwd1, newpwd2) {
			var thisWidget = this;
			if (com.beas.pwd.config && com.beas.pwd.config.url && com.beas.pwd.config.url.length > 0 ) {
				var url = com.ibm.social.incontext.util.proxy(com.beas.pwd.config.url + "/resetpassword");

				var xhrArgs = {
						url: url,
						postData: "oldpassword=" + encodeURIComponent(oldpwd) + "&newpassword1=" + encodeURIComponent(newpwd1) + "&newpassword2=" + encodeURIComponent(newpwd2),
						handleAs: "json",
						load: function(data){
							var severity = "error";
